...
1. When using Git locally, what are these?  Define each one in a sentence
   * Staging area - ``` Staging area contains files and changes that are already added, but not yet committed. ```
   * Working copy - ``` Working copy is a local copy of a Git repository, which is the copy of files you work on. ```
   * master - ``` master is the name of a default branch in a Git repository. ```
   * HEAD - ``` HEAD refers to the commit that the working copy is based on. ```

2. When you install git on a new machine (or in a new user account) you should perform these 2 git commands to tell git your name and email.  These values are used in commits that you make:
   ```
   # Git configuration commands for a new account
   git config --global user.name "Jullaphong Jiamwatthanaloet" 
   git config --global user.email jullphong.j@ku.th
   ```

3. There are 2 ways to create a local Git repository.  Briefly describe each one:
   - Create a new Git repository in an existed directory.
   ```
   git init
   ```
   
   - Clone a remote Git repository to current local directory.
   ```
...
1. Create a new branch and switch to that new branch.
   ```
...
3. Rebase ```dev-test``` branch onto ```master``` branch.
   ```
...
1. Create a new branch from a specific commit. This starts a new branch at the specific commit, and automatically switch to that branch.
   ```
   git checkout -b new-branch commit-hash
   ```
   
2. Undo a specific commit while safely keep the unwanted commit in the history.
   ```
...
* [Git Cheat Sheet](https://www.atlassian.com/git/tutorials/atlassian-git-cheatsheet) useful git cheat sheet to look up git commands at a glance.
* [Git and GitHub Tutorial](https://www.w3schools.com/git/) handfuls of tutorial for git and GitHub. 
