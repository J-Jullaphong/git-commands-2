...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > This error occurs because there is no `dev-foo` branch in the remote repository. For that reason, the local `dev-foo` branch will not be able to track changes and update files on the remote repository.  
